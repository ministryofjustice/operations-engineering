...
def alert_on_low_quota():
    ...
    github_service = GithubService(
        github_token, MINISTRY_OF_JUSTICE, ENTERPRISE
    )
    ...
    if check_result is not False:
        slack_service.send_message_to_plaintext_channel_name(
            low_threshold_triggered_message(check_result["percentage_used"]),
            SLACK_CHANNEL,
        )
        github_service.modify_gha_minutes_quota_threshold(
            check_result["threshold"] + 10
        )
