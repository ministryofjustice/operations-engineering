...
def identify_dormant_github_users():
    ...
    dormant_users_according_to_github_and_auth0 = [user for user in dormant_users_according_to_github if user.is_dormant]

    SlackService(env.get('ADMIN_SLACK_TOKEN')).send_message_to_plaintext_channel_name(message_to_slack_channel(dormant_users_according_to_github_and_auth0), SLACK_CHANNEL)
