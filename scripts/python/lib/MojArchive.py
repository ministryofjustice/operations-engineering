...
class MojArchive:
    ...
    def archive(self) -> bool:
        ...
        if self.is_on_allow_list:
            logging.info(f"Skipping repository:  {self.repo.name}, Reason: Present in allow list ")
            return True
        ...
        if self.archived:
            self.complete = True
            logging.info(f"Archiving repository: {self.repo.name}, Status: Successful")
            return True
        else:
            logging.error(f"Archiving repository: {self.repo.name}, Status: Failure")
            return False