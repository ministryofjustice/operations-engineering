...
class GithubService:
    ...
    @retries_github_rate_limit_exception_at_next_reset_once
    def get_paginated_list_of_org_repository_names_and_first_n_topics(
        self,
        n_topics: int,
        after_cursor: str | None,
        page_size: int = GITHUB_GQL_DEFAULT_PAGE_SIZE,
        ...
        logging.info(
            f"Getting paginated list of org repository names and first {n_topics} topics. Page size {page_size}, after cursor {bool(after_cursor)}")
        if page_size > self.GITHUB_GQL_MAX_PAGE_SIZE:
            raise ValueError(
                f"Page size of {page_size} is too large. Max page size {self.GITHUB_GQL_MAX_PAGE_SIZE}")
        return self.github_client_gql_api.execute(gql("""
            query($organisation_name: String!, $page_size: Int!, $after_cursor: String, $n_topics: Int!) {
                organization(login: $organisation_name) {
                    repositories(first: $page_size, after: $after_cursor) {
                        pageInfo {
                            endCursor
                        ...
                        edges {
                            node {
                                ...
                                isLocked
                                name
                                repositoryTopics(first: $n_topics) {
                                    edges {
                                        node {
                                            topic {
                                                name
        ...
        """), variable_values={"organisation_name": self.organisation_name, "n_topics": n_topics,
                               "page_size": page_size, "after_cursor": after_cursor})


    @retries_github_rate_limit_exception_at_next_reset_once
    def get_org_repo_name_and_first_n_topics(self, n_topics=10) -> list[str]:
        """A wrapper function to run a GraphQL query to get a list of the organisation 
        repository names and topics

        Returns:
            list: A list of lists of the organisation repository names and their first n topics
        """
        has_next_page = True
        after_cursor = None
        repository_names_and_topics = []
        while has_next_page:
            data = self.get_paginated_list_of_org_repository_names_and_first_n_topics(
                n_topics, after_cursor, 100)
            ...
            if data["organization"]["repositories"]["edges"] is not None:
                for repo in data["organization"]["repositories"]["edges"]:
                    if repo["node"]["isDisabled"] == True or repo["node"]["isArchived"] == True or repo["node"][
                            "isLocked"] == True:
                        continue
                    repository_names_and_topics.append(
                        [
                            repo["node"]["name"],
                            [t["node"]["topic"]["name"] for t in repo["node"]["repositoryTopics"]["edges"]]
                        ]
                    )

            has_next_page = data["organization"]["repositories"]["pageInfo"]["hasNextPage"]
            after_cursor = data["organization"]["repositories"]["pageInfo"]["endCursor"]
        return repository_names_and_topics
