...
class GithubService:
    ...
    def archive_all_inactive_repositories(
        self, last_active_cutoff_date: datetime, allow_list: list[str]
    ) -> None:
    ...
    def __get_repos_to_consider_for_archiving(
        self, repository_type: str
    ) -> list[Repository]:
        repositories = list(
            self.github_client_core_api.get_organization(
                self.organisation_name
            ).get_repos(type=repository_type)
        )
        return [
            repository
    ...
    def get_users_of_multiple_organisations(self, organisations: list) -> list:
        ...
        for org in organisations:
            all_users = all_users + [
                user["login"]
                for user in self.github_client_core_api.get_organization(
                    org
                ).get_members()
    ...
    @retries_github_rate_limit_exception_at_next_reset_once
    def get_paginated_list_of_org_repository_names(
    ...
    def get_org_repo_names(self) -> list[str]:
        ...
        while has_next_page:
            ...
            has_next_page = data["organization"]["repositories"]["pageInfo"][
                "hasNextPage"
            ]
    ...
    def set_standards(self, repository_name: str):
        ...
        branch.edit_protection(
            contexts=current_protection.required_status_checks.contexts
            if current_protection.required_status_checks
            else [],
    ...
    @retries_github_rate_limit_exception_at_next_reset_once
    def get_paginated_list_of_repositories_per_topic(
        self,
        topic: str,
        after_cursor: str | None,
        page_size: int = GITHUB_GQL_DEFAULT_PAGE_SIZE,
    ...
    def close_repository_open_issues_with_tag(self, repository: str, tag: str) -> None:
        logging.info(f"Closing open issues in repository {repository} with tag {tag}")
        open_issues = self.github_client_core_api.get_repo(
            f"{self.organisation_name}/{repository}"
    ...
    def get_audit_log_active_users(self, users: list) -> list:
        ...
        active_users = []
        for user in users:
            audit_log_data = self._get_user_from_audit_log(user["username"])
    ...
    @retries_github_rate_limit_exception_at_next_reset_once
    def check_dormant_users_audit_activity_since_date(
        self, users: list, since_date: datetime
    ) -> list:
        return [
            user for user in users if self.is_user_dormant_since_date(user, since_date)
        ]
    ...
    def remove_user_from_gitub(self, user: str):
        ...
        self.github_client_core_api.get_organization(
            self.organisation_name
        ).remove_from_membership(github_user)
    ...
    def _identify_inactive_users(
        self,
        users: list[NamedUser.NamedUser],
    ...
    def _get_unignored_users_from_team(
        self, team_id: int, users_to_ignore: list[str]
    ) -> list[NamedUser.NamedUser]:
    ...
    def _get_repositories_managed_by_team(
        self, team_id: int, repositories_to_ignore: list[str]
    ) -> list[Repository]:
        ...
        return [
            ...
            if repo.name.lower() not in repositories_to_ignore
        ]

    def _is_user_inactive(
        self,
        ...
        cutoff_date = datetime.now() - timedelta(
            days=inactivity_months * 30
        )  # Roughly calculate the cutoff date
    ...
    def remove_list_of_users_from_team(
        self, team_name: str, users: list[NamedUser.NamedUser]
    ) -> None:
        """
    ...
    @retries_github_rate_limit_exception_at_next_reset_once
    def _get_paginated_organization_members_with_emails(
        self, after_cursor: str | None, page_size: int = GITHUB_GQL_MAX_PAGE_SIZE
    ) -> dict[str, Any]:
        logging.info(
    ...
    def get_github_member_list(self):
        ...
        while next_page:
            ...
            if (
                ...
                and "membersWithRole" in response["organization"]
            ):
                ...
                for member in all_members:
                    email = (
                        member["organizationVerifiedDomainEmails"][0]
                        if member["organizationVerifiedDomainEmails"]
                        else None
                    )
                    github_usernames.append(
                        {"username": member["login"], "email": email}
                    )
    ...
    def update_team_repository_permission(
        self, team_name: str, repositories, permission: str
    ):
    ...
    def flag_owner_permission_changes(self, since_date: str) -> list:
        ...
        for change in recent_changes:
            match change["action"]:
                ...
                case "org.update_member" if change["permission"] == "ADMIN" and change[
                    "permissionWas"
                ] == "READ":
                    list_of_changes_to_flag.append(change)
                case _:
                    logging.info(f"Change {change} does not meet criteria to flag")
    ...
    def audit_log_member_changes(self, since_date: str) -> list:
        ...
        while True:
            ...
            all_entries.extend(
                [
                    entry["node"]
                    for entry in data["organization"]["auditLog"]["edges"]
                    if entry["node"]
                ]
            )

            if data["organization"]["auditLog"]["pageInfo"]["hasNextPage"]:
                variable_values["cursor"] = data["organization"]["auditLog"][
                    "pageInfo"
                ]["endCursor"]
            else:
                break
    ...
    def get_all_organisations_in_enterprise(self) -> list[Organization]:
        ...
        return [
            org.login for org in self.github_client_core_api.get_user().get_orgs()
        ] or []
    ...
    def modify_gha_minutes_quota_threshold(self, new_threshold):
        ...
        self.github_client_rest_api.patch(
            "https://api.github.com/repos/ministryofjustice/operations-engineering/actions/variables/GHA_MINUTES_QUOTA_THRESHOLD",
            json.dumps(payload),
            headers=headers,
        )
    ...
    def get_gha_minutes_quota_threshold(self):
        ...
        ).get_variable("GHA_MINUTES_QUOTA_THRESHOLD")
    ...
    def detect_neglected_files(self):
        last_reviewed_date = (
            ...
            .get_commits(path="bin/add_users_all_org_members_github_team.py")[0]
