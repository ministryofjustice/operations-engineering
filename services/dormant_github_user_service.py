...
class DormantGitHubUser:
    def __init__(
        ...
        name: str,
        days_considered_dormant: int = 90,
    ):
    ...
    def email(self):
        if self._email is None:
            self._email = self.github_service.get_user_org_email_address(self.name)
        return self._email

    @property
    def last_github_activity(self):
        if self._last_github_activity is None:
            self._last_github_activity = (
                self.github_service.get_last_audit_log_activity_date_for_user(self.name)
            )
    ...
    def is_dormant(self) -> bool:
        ...
        return (
            self.last_github_activity < self.threshold
            and self.last_auth0_activity < self.threshold
        )
    ...
    def format_timestamp(self, timestamp):
        return datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S.%f").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
