...
class TestMojGithub(unittest.TestCase):
    ...
    def test_repos(self):
        self.org.get_repos.return_value = [Mock(archived=False, fork=False), Mock(archived=True, fork=False)]
    ...
    def test_get_unarchived_repos(self):
        self.org.get_repos.return_value = [Mock(archived=False, fork=False, repo_type="public"), Mock(archived=True, fork=False, repo_type="private")]
        ...
        self.assertEqual(len(self.mojgithub.get_unarchived_repos("private")), 1)

    def test_exclude_archived_repos(self):
        repos = [Mock(archived=False, fork=False), Mock(archived=True, fork=False)]
        self.assertEqual(len(self.mojgithub._exclude_archived_repos(repos)), 1)

    def test_exclude_fork_repos(self):
        repos = [Mock(archived=False, fork=False), Mock(archived=False, fork=True)]
